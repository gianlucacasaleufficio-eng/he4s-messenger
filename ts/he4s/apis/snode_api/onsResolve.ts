...
async function getHE4SIDForOnsName(onsNameCase: string) {
  ...
  const promises = range(0, validationCount).map(async () => {
    ...
    const he4sIDAsData = sodium.crypto_aead_xchacha20poly1305_ietf_decrypt(
      null,
      ciphertext,
      null,
      nonce,
    ...
    return toHex(he4sIDAsData);
  });

  try {
    // if one promise throws, we end un the catch case
    const allResolvedHE4SIds = await Promise.all(promises);
    if (allResolvedHE4SIds?.length !== validationCount) {
      throw new Error('ONSresolve: Validation failed');
    }

    // assert all the returned account ids are the same
    if (_.uniq(allResolvedHE4SIds).length !== 1) {
      throw new Error('ONSresolve: Validation failed');
    }
    return allResolvedHE4SIds[0];
  } catch (e) {
...
export const ONSResolve = { onsNameRegex, getHE4SIDForOnsName };
