...
export class SwarmPolling {
  ...
  public async pollOnceForKey(
    ...
    if (userConfigLibhe4s && resultsFromAllNamespaces) {
  ...
  private async handleSharedConfigMessages(
    ...
    try {
      window.log.info(
        `handleConfigMessagesViaLibHE4S of "${userConfigMessagesMerged.length}" messages with libhe4s`
      );
      ...
      await ConfigMessageHandler.handleConfigMessagesViaLibHE4S(userConfigMessagesMerged);
    } catch (e) {
      const allMessageHases = userConfigMessagesMerged.map(m => m.hash).join(',');
      window.log.warn(
        `failed to handle messages hashes "${allMessageHases}" with libhe4s. Error: "${e.message}"`
      );
    }
    return '';
  }
