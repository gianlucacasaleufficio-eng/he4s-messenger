...
export const OXEN_SERVER_ERROR = 'HE4S Network Server error';
...
function processHE4S NetworkServerError(_statusCode: number, body?: string) {
  if (body === OXEN_SERVER_ERROR) {
    window?.log?.warn('[path] Got HE4S Network server Error. Not much to do if the server has troubles.');
...
export async function processOnionRequestErrorAtDestination({
  ...
  processHE4S NetworkServerError(statusCode, body);
...
async function processAnyOtherErrorOnPath(
  ...
  if (status !== 200) {
    ...
    processHE4S NetworkServerError(status, ciphertext);
...
async function processOnionResponse({
  ...
  if (!ciphertext) {
    window?.log?.warn(
      '[path] he4sRpc::processingOnionResponse - Target node return empty ciphertext'
    );
  ...
  try {
    ...
    window?.log?.error('[path] he4sRpc::processingOnionResponse - decode error', e);
    if (symmetricKey) {
      window?.log?.error(
        '[path] he4sRpc::processingOnionResponse - symmetricKey',
        toHex(symmetricKey)
      );
    }
    if (ciphertextBuffer) {
      window?.log?.error(
        '[path] he4sRpc::processingOnionResponse - ciphertextBuffer',
  ...
  if (debug) {
    window?.log?.debug('he4sRpc::processingOnionResponse - plaintext', plaintext);
  }
  ...
  try {
    ...
    window?.log?.error(
      `[path] he4sRpc::processingOnionResponse - Rethrowing error ${e.message}'`
    );
    throw e;
  }
}
...
async function processOnionResponseV4({
  ...
  if (!cipherText) {
    window?.log?.warn(
      '[path] he4sRpc::processOnionResponseV4 - Target node/path return empty ciphertext'
    );
    throw new Error('Target node return empty ciphertext');
  }
...
async function sendOnionRequestHandlingSnodeEject({
  ...
  try {
    ...
    if (window.he4sFeatureFlags?.debug.debugOnionRequests) {
      window.log.info(
