...
export async function TEST_fetchFromSeedWithRetriesAndWriteToDb() {
  ...
  if (!seedNodes || !seedNodes.length) {
    window?.log?.error(
      'HE4SSnodeAPI:::fetchFromSeedWithRetriesAndWriteToDb - getSeedNodeList has not been loaded yet'
    );
  ...
  try {
    ...
    window?.log?.error(
      'HE4SSnodeAPI:::fetchFromSeedWithRetriesAndWriteToDb - Failed to fetch snode poll from seed node with retries. Error:',
      e
    );
  }
}
