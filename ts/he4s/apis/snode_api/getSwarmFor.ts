...
async function requestSnodesForPubkeyWithTargetNodeRetryable(
  ...
  if (!result || !result.length) {
    window?.log?.warn(
      `HE4SSnodeAPI::requestSnodesForPubkeyWithTargetNodeRetryable - he4sRpc on ${targetNode.ip}:${targetNode.port} returned falsish value`,
      result
    );
  ...
  try {
    ...
    if (!body.snodes || !isArray(body.snodes) || !body.snodes.length) {
      window?.log?.warn(
        `HE4SSnodeAPI::requestSnodesForPubkeyRetryable - he4sRpc on ${targetNode.ip}:${targetNode.port} returned falsish value for snodes`,
        result
      );
...
export async function requestSnodesForPubkeyFromNetwork(pubKey: string): Promise<Array<Snode>> {
  try {
    ...
    window?.log?.error('HE4SSnodeAPI::requestSnodesForPubkey - error', e);
