...
export type SubRequestAddRemoveModeratorType = {
  ...
  addRemoveModerators: {
    ...
    he4sIds: Array<string>; // can be blinded id or not
    roomId: string; // for now we support only granting/removing mods to single rooms from he4s
  };
};
...
export type SubRequestBanUnbanUserType = {
  ...
  banUnbanUser: {
    type: 'ban' | 'unban';
    he4sId: string; // can be blinded id or not
    roomId: string;
  };
};
...
const makeBatchRequestPayload = (
  ...
  switch (type) {
    ...
    case 'addRemoveModerators':
      ...
      return options.addRemoveModerators.he4sIds.map(he4sId => ({
        method: 'POST',
        path: `/user/${he4sId}/moderator`,

        // An admin has moderator permissions automatically, but removing his admin permissions only will keep him as a moderator.
        // We do not want this currently. When removing an admin from HE4S Desktop we want to remove all his permissions server side.
    ...
    case 'banUnbanUser':
      ...
      return {
        method: 'POST',
        path: `/user/${options.banUnbanUser.he4sId}/${isBan ? 'ban' : 'unban'}`,
        json: {
    ...
    case 'deleteAllPosts':
      return {
        method: 'DELETE',
        path: `/room/${options.deleteAllPosts.roomId}/all/${options.deleteAllPosts.he4sId}`,
      };
    case 'updateRoom':
      return {
        method: 'PUT',
