...
export function getCachedNakedKeyFromBlinded(
  ...
  return found?.realHE4SId || undefined;
}
...
export async function addCachedBlindedKey({
  ...
  if (!isNonBlindedKey(realHE4SId)) {
    throw new Error('realHE4SId must not be blinded');
  }
  ...
  if (foundIndex >= 0) {
    if (assertLoadedCache[foundIndex].realHE4SId !== realHE4SId) {
      window.log.warn(
        `overriding cached blinded mapping for ${assertLoadedCache[foundIndex].realHE4SId} with ${realHE4SId} on ${serverPublicKey}`
      );
      assertLoadedCache[foundIndex].realHE4SId = realHE4SId;
      await writeKnownBlindedKeys();
    }

    return;
  }
  assertLoadedCache.push({ blindedId, serverPublicKey, realHE4SId });
  await writeKnownBlindedKeys();
}

/**
 * Only exported for testing
 * Try to match a blindedId with a standardHE4SID. This is the only way we have to find that a standard and a blindedID are in fact, the same person.
 */
export function tryMatchBlindWithStandardKey(
  standardHE4SId: string,
  blindedHE4SId: string,
  serverPubKey: string,
  sodium: LibSodiumWrappers
): boolean {
  if (!standardHE4SId.startsWith(KeyPrefixType.standard)) {
    throw new Error('standardKey must be a standard key (starting with 05)');
  }

  if (!PubKey.isBlinded(blindedHE4SId)) {
  ...
  try {
    const he4sIdNoPrefix = PubKey.removePrefixIfNeeded(PubKey.cast(standardHE4SId).key);
    const blindedIdNoPrefix = PubKey.removePrefixIfNeeded(PubKey.cast(blindedHE4SId).key);
    ...
    const inbin = from_hex(he4sIdNoPrefix);
 ...
 * If the given pubkey is not blinded, it returns true if it is our naked HE4SID.
...
export function isUsAnySogsFromCache(blindedOrNakedId: string): boolean {
  ...
  const found = assertLoaded().find(
    m => m.blindedId === blindedOrNakedId && m.realHE4SId === usUnblinded
  );
...
export function getUsBlindedInThatServer(convo: ConversationModel | string): string | undefined {
  ...
  const found = assertLoaded().find(
    m => m.serverPublicKey === room.serverPublicKey && m.realHE4SId === usNaked
  );
  return found?.blindedId;
}

/**
 * This function can be called to find all blinded conversations we have with a user given its real he4sID.
...
export async function findCachedBlindedMatchOrLookItUp(
  ...
  const realHE4SIdFound = findNotCachedBlindingMatch(blindedId, serverPubKey, sodium);

  if (realHE4SIdFound) {
    await addCachedBlindedKey({
      blindedId,
      realHE4SId: realHE4SIdFound,
      serverPublicKey: serverPubKey,
    });
    return realHE4SIdFound;
  }
  return undefined;
}
...
export function findCachedBlindedIdFromUnblinded(
  ...
  const found = assertLoaded().find(
    m => m.serverPublicKey === serverPubKey && m.realHE4SId === unblindedId
  );
...
export async function findCachedOurBlindedPubkeyOrLookItUp(
  ...
  const ourNakedHE4SID = UserUtils.getOurPubKeyStrFromCache();

  if (PubKey.isBlinded(ourNakedHE4SID)) {
    throw new Error('findCachedBlindedIdFromUnblindedOrLookItUp needs a unblindedID');
  }
  let found = findCachedBlindedIdFromUnblinded(ourNakedHE4SID, serverPubKey);
  ...
  found = findCachedBlindedIdFromUnblinded(ourNakedHE4SID, serverPubKey);
