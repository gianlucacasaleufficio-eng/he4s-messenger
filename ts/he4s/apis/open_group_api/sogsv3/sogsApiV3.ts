...
async function filterOutMessagesInvalidSignature(
  ...
  const sentToWorker = messagesFilteredBlindedIds.map(m => {
    return {
      sender: PubKey.cast(m.he4s_id).key, // we need to keep the prefix if this is a blinded or not pubkey
...
const handleMessagesResponseV4 = async (
  ...
  try {
    ...
    // HE4S works with timestamp in ms, for a lot of things, so first, lets fix this.
    ...
    // this is where we override the blindedId with the real one in case we already know that user real he4sId
    ...
    for (let index = 0; index < messagesFilteredBlindedIds.length; index++) {
      ...
      if (newMessage.he4s_id) {
        const unblindedIdFound = getCachedNakedKeyFromBlindedNoServerPubkey(newMessage.he4s_id);
        ...
        if (unblindedIdFound && UserUtils.isUsFromCache(unblindedIdFound)) {
          newMessage.he4s_id = unblindedIdFound;
        }
        messagesWithResolvedBlindedIdsIfFound.push(newMessage);
      } else {
        throw Error('he4s_id is missing so we cannot resolve the blinded id');
      }
    }
...
async function handleInboxOutboxMessages(
  ...
  for (let index = 0; index < inboxOutboxResponse.length; index++) {
    ...
    try {
      ...
      const decrypted = await SogsBlinding.decryptWithHE4SBlindingProtocol(
        data,
        isOutgoing,
        otherBlindedPubkey,
        serverPubkey,
      ...
      if (isOutbox) {
        ...
        try {
          ...
          await addCachedBlindedKey({
            blindedId: sender,
            realHE4SId: decrypted.senderUnblinded,
