...
export const filterDuplicatesFromDbAndIncomingV4 = async (
  ...
  }).filter(m => Boolean(m.he4s_id && m.posted));
  ...
  const filteredInDb = await Data.filterAlreadyFetchedOpengroupMessage(
    filtered.map(m => {
      ...
      return { sender: m.he4s_id! as string, serverTimestamp: m.posted! };
    })
  );
  ...
  const opengroupMessagesV4Filtered = filteredInDb?.map(f => {
    return newMessages.find(m => m.he4s_id === f.sender && m.posted === f.serverTimestamp);
  });
