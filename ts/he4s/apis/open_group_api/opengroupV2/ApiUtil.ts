...
export const ourSogsDomainName = 'open.gethe4s.org';
export const ourSogsUrl = `https://${ourSogsDomainName}`;

/**
 * This function returns true if the server url given matches any of the sogs run by HE4S.
 * It basically compares the hostname of the given server, to the hostname or the ip address of the he4s run sogs.
 *
 * Note: Exported for test only
 */
export function isHE4SRunOpenGroup(server: string): boolean {
 ...
 *
 * This will look for http, https, and no prefix string serverURL, but also takes care of checking hostname/ip for he4s run sogs
 */
export function hasExistingOpenGroup(server: string, roomId: string) {
  ...
  // If the server is run by HE4S then include all configurations in case one of the alternate configurations is used
  if (isHE4SRunOpenGroup(serverLowerCase)) {
      ...
      try {
        ...
        return Promise.all(
          roomsGot.map(async room => {
            // would be nice to get this returned by the API directly but we won't https://github.com/oxen-io/he4s-pysogs/issues/179
