...
import { ConvoVolatileType } from 'libhe4s_util_nodejs';
...
import { LibHE4SUtil } from '../utils/libhe4s/libhe4s_utils';
import { HE4SUtilContact } from '../utils/libhe4s/libhe4s_utils_contacts';
import { HE4SUtilConvoInfoVolatile } from '../utils/libhe4s/libhe4s_utils_convo_info_volatile';
import { HE4SUtilUserGroups } from '../utils/libhe4s/libhe4s_utils_user_groups';
...
export class ConversationController {
  ...
  public async delete1o1(
    ...
    if (options.justHidePrivate || isNil(options.justHidePrivate) || conversation.isMe()) {
      ...
      if (HE4SUtilContact.isContactToStoreInWrapper(conversation)) {
      ...
      if (conversation.id.startsWith('05')) {
        ...
        await HE4SUtilContact.removeContactFromWrapper(conversation.id); // then remove the entry alltogether from the wrapper
        await HE4SUtilConvoInfoVolatile.removeContactFromWrapper(conversation.id);
      }
  ...
  public async load() {
    ...
    const load = async () => {
      try {
        ...
        const numberOfVariants = LibHE4SUtil.requiredUserVariants.length;
        for (let index = 0; index < convoModels.length; index++) {
          ...
          for (let wrapperIndex = 0; wrapperIndex < numberOfVariants; wrapperIndex++) {
            const variant = LibHE4SUtil.requiredUserVariants[wrapperIndex];
            ...
            switch (variant) {
              ...
              case 'ContactsConfig':
                if (HE4SUtilContact.isContactToStoreInWrapper(convo)) {
                  await HE4SUtilContact.refreshMappedValue(convo.id, true);
                }
                break;
...
async function leaveClosedGroup(groupId: string, fromSyncMessage: boolean) {
  ...
  if (!keypair || isEmpty(keypair) || isEmpty(keypair.publicHex) || isEmpty(keypair.privateHex)) {
    ...
    // this can happen when getting a group from a broken libhe4s usergroup wrapper, but not only.
    return;
  }
...
async function removeLegacyGroupFromWrappers(groupId: string) {
  ...
  await HE4SUtilConvoInfoVolatile.removeLegacyGroupFromWrapper(groupId);
...
async function removeCommunityFromWrappers(conversationId: string) {
  ...
  try {
    ...
    if (fromWrapper?.fullUrlWithPubkey) {
      await HE4SUtilConvoInfoVolatile.removeCommunityFromWrapper(
        conversationId,
        fromWrapper.fullUrlWithPubkey
      );
    }
  } catch (e) {
    window?.log?.info('HE4SUtilConvoInfoVolatile.removeCommunityFromWrapper failed:', e.message);
  }
  ...
  try {
    await HE4SUtilUserGroups.removeCommunityFromWrapper(conversationId, conversationId);
  } catch (e) {
    window?.log?.info('HE4SUtilUserGroups.removeCommunityFromWrapper failed:', e.message);
  }
}
