...
export class PubKey {
   ...
   *
   * Note: this should be used when starting a conversation and we do not support starting conversation from scratch with a blinded he4sId.
  ...
  public static validateWithErrorNoBlinding(pubkey: string): string | undefined {
    ...
    // dev pubkey on testnet are now 66 chars too with the prefix, so every he4sID needs 66 chars and the prefix to be valid
    if (!isProdOrDevValid) {
      return window.i18n('accountIdErrorInvalid');
    }
    return undefined;
