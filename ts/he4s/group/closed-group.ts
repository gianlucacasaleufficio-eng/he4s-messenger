...
import { encryptUsingHE4SProtocol } from '../crypto/MessageEncrypter';
...
async function generateAndSendNewEncryptionKeyPair(
  ...
  const messageSentCallback = async () => {
    ...
    await groupConvo?.commit(); // this makes sure to include the new encryption keypair in the libhe4s usergroup wrapper
  };
...
export async function buildEncryptionKeyPairWrappers(
  ...
  const wrappers = await Promise.all(
    targetMembers.map(async pubkey => {
      const ciphertext = await encryptUsingHE4SProtocol(PubKey.cast(pubkey), plaintext);
