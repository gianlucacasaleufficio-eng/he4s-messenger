...
export async function getGuardNodeOrSelectNewOnes() {
  if (guardNodes.length === 0) {
    ...
    if (guardNodesFromDb.length === 0) {
      window?.log?.warn(
        'HE4SSnodeAPI::getGuardNodeOrSelectNewOnes - no guard nodes in DB. Will be selecting new guards nodes...'
      );
    } else {
      ...
      if (guardNodes.length < edKeys.length) {
        window?.log?.warn(
          `HE4SSnodeAPI::getGuardNodeOrSelectNewOnes - could not find some guard nodes: ${guardNodes.length}/${edKeys.length} left`
        );
      }
    }
  }
...
async function buildNewOnionPathsWorker() {
  return pRetry(
    async () => {
      window?.log?.info('HE4SSnodeAPI::buildNewOnionPaths - building new onion paths...');
      ...
      window?.log?.info(
        `HE4SSnodeAPI::buildNewOnionPaths, snodePool length: ${allNodes.length}`
      );
