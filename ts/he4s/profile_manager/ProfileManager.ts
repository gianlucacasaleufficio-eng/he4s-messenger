...
import { UserConfigWrapperActions } from '../../webworker/workers/browser/libhe4s_worker_interface';
import { getConversationController } from '../conversations';
import { SyncUtils, UserUtils } from '../utils';
import { fromHexToArray, sanitizeHE4SUsername, toHex } from '../utils/String';
 ...
 * That limit is enforced by libhe4s when calling `setName` on the `UserConfigWrapper`.
...
async function updateOurProfileDisplayNameOnboarding(newName: string) {
  const cleanName = sanitizeHE4SUsername(newName).trim();

  try {
    // create a temp user config wrapper to test the display name with libhe4s
...
async function updateOurProfileDisplayName(newName: string) {
  ...
  await UserConfigWrapperActions.setNameTruncated(sanitizeHE4SUsername(newName).trim());
  ...
  conversation.setHE4SDisplayNameNoCommit(truncatedName);
