  ...
  const contacts = contactsModels.map(c => {
    try {
      ...
      return new ConfigurationMessageContact({
        ...
        displayName: c.getRealHE4SUsername() || 'Anonymous',
...
export const getCurrentConfigurationMessage = async (
  ...
  const displayName = ourConvo?.getRealHE4SUsername() || 'Anonymous'; // this should never be undefined, but well...
