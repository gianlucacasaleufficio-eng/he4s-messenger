...
import { CommunityInfo, LegacyGroupInfo, UserGroupsType } from 'libhe4s_util_nodejs';
...
async function insertGroupsFromDBIntoWrapperAndRefresh(
  ...
  if (!HE4SUtilUserGroups.isUserGroupToStoreInWrapper(foundConvo)) {
    return null;
  }

  const convoType: UserGroupsType = HE4SUtilUserGroups.isCommunityToStoreInWrapper(foundConvo)
    ? 'Community'
    : 'LegacyGroup';
...
export const HE4SUtilUserGroups = {
  // shared
