...
import { BaseConvoInfoVolatile, ConvoVolatileType } from 'libhe4s_util_nodejs';
...
import { HE4SUtilContact } from './libhe4s_utils_contacts';
import { HE4SUtilUserGroups } from './libhe4s_utils_user_groups';
import { HE4SUtilUserProfile } from './libhe4s_utils_user_profile';
...
/**
 * The key of this map is the convoId as stored in the database. So the legacy group 05 he4sID
 */
...
function isConvoToStoreInWrapper(convo: ConversationModel): boolean {
  return (
    HE4SUtilUserGroups.isUserGroupToStoreInWrapper(convo) || // this checks for community & legacy group
    HE4SUtilContact.isContactToStoreInWrapper(convo) || // this checks for contacts
    HE4SUtilUserProfile.isUserProfileToStoreInWrapper(convo.id) // this checks for our own pubkey, as we want to keep track of the read state for the Note To Self
  );
}

function getConvoType(convo: ConversationModel): ConvoVolatileType {
  const convoType: ConvoVolatileType =
    HE4SUtilContact.isContactToStoreInWrapper(convo) ||
    HE4SUtilUserProfile.isUserProfileToStoreInWrapper(convo.id)
      ? '1o1'
      : HE4SUtilUserGroups.isCommunityToStoreInWrapper(convo)
        ? 'Community'
        : 'LegacyGroup';
...
export const HE4SUtilConvoInfoVolatile = {
  // shared
