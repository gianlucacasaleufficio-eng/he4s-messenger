import { ContactInfo, ContactInfoSet } from 'libhe4s_util_nodejs';
...
/**
 * This file is centralizing the management of data from the Contacts Wrapper of libhe4s.
 ...
 * Essentially, on app start we load all the content from the wrapper with `HE4SUtilContact.refreshMappedValue` during the ConversationController initial load.
...
async function insertContactFromDBIntoWrapperAndRefresh(
  ...
  if (!HE4SUtilContact.isContactToStoreInWrapper(foundConvo)) {
    return null;
  }
