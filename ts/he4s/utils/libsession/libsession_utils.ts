...
import { GenericWrapperActions } from '../../../webworker/workers/browser/libhe4s_worker_interface';
...
/**
 * Initializes the libhe4s wrappers for the required user variants if the dumps are not already in the database. It will use an empty dump if the dump is not found.
 */
async function initializeLibHE4SUtilWrappers() {
  ...
  window.log.info(
    'initializeLibHE4SUtilWrappers alldumpsInDB already: ',
  ...
  for (let index = 0; index < dumps.length; index++) {
    ...
    window.log.debug('initializeLibHE4SUtilWrappers initing from dump', dump.variant);
    try {
      ...
      throw new Error(`initializeLibHE4SUtilWrappers failed with ${e.message}`);
    }
  }

  const missingRequiredVariants: Array<ConfigWrapperObjectTypes> = difference(
    LibHE4SUtil.requiredUserVariants,
  ...
  for (let index = 0; index < missingRequiredVariants.length; index++) {
    ...
    window.log.warn(
      `initializeLibHE4SUtilWrappers: missingRequiredVariants "${missingVariant}"`
    );
    ...
    window.log.debug(
      `initializeLibHE4SUtilWrappers: missingRequiredVariants "${missingVariant}" created`
    );
  }
}
...
async function pendingChangesForPubkey(pubkey: string): Promise<Array<OutgoingConfResult>> {
  ...
  if (pubkey === us) {
    LibHE4SUtil.requiredUserVariants.forEach(requiredVariant => {
...
export const LibHE4SUtil = {
  initializeLibHE4SUtilWrappers,
