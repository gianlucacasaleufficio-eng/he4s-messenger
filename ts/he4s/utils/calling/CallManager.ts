...
/**
 * This array holds all of the ice servers HE4S can contact.
...
const iceServersFullArray = [
  {
    urls: 'turn:freyr.gethe4s.org',
    username: 'he4s202111',
    credential: '053c268164bc7bd7',
  },
  // excluding those two (fenrir & frigg) as they are TCP only for now
  // {
  //   urls: 'turn:fenrir.gethe4s.org',
  //   username: 'he4s202111',
  //   credential: '053c268164bc7bd7',
  // },
  // {
  //   urls: 'turn:frigg.gethe4s.org',
  //   username: 'he4s202111',
  //   credential: '053c268164bc7bd7',
  // },
  {
    urls: 'turn:angus.gethe4s.org',
    username: 'he4s202111',
    credential: '053c268164bc7bd7',
  },
  {
    urls: 'turn:hereford.gethe4s.org',
    username: 'he4s202111',
    credential: '053c268164bc7bd7',
  },
  {
    urls: 'turn:holstein.gethe4s.org',
    username: 'he4s202111',
    credential: '053c268164bc7bd7',
  },
  {
    urls: 'turn:brahman.gethe4s.org',
    username: 'he4s202111',
    credential: '053c268164bc7bd7',
  },
];
...
function createOrGetPeerConnection(withPubkey: string) {
  ...
  dataChannel = peerConnection.createDataChannel('he4s-datachannel', {
    ordered: true,
    negotiated: true,
...
export async function USER_acceptIncomingCallRequest(fromSender: string) {
  ...
  try {
    await peerConnection.setRemoteDescription(
      new RTCHE4SDescription({
        sdp: sdps[0],
        type: 'offer',
      })
    );
  } catch (e) {
    window.log?.error(`Error setting RTC HE4S Description ${e}`);
  }
...
export async function handleCallTypeOffer(
  ...
  try {
    ...
    if (peerConnection && remoteCallUUID === currentCallUUID && currentCallUUID) {
      ...
      const remoteOfferDesc = new RTCHE4SDescription({
        type: 'offer',
...
export async function handleMissedCall(
  ...
  const displayname =
    ...
    incomingCallConversation?.getRealHE4SUsername() ||
    window.i18n('unknown');
...
export async function handleCallTypeAnswer(
  ...
  try {
    ...
    const remoteDesc = new RTCHE4SDescription({
      type: 'answer',
      sdp: callMessage.sdps[0],
    });
