...
import { LibHE4SUtil, OutgoingConfResult } from '../../libhe4s/libhe4s_utils';
...
async function retrieveSingleDestinationChanges(
  ...
  const outgoingConfResults = await LibHE4SUtil.pendingChangesForPubkey(destination);
...
async function buildAndSaveDumpsToDB(
  ...
  for (let i = 0; i < changes.length; i++) {
    ...
    const variant = LibHE4SUtil.kindToVariant(change.message.kind);

    const needsDump = await LibHE4SUtil.markAsPushed(
      variant,
      destination,
...
async function saveDumpsNeededToDB(destination: string) {
  for (let i = 0; i < LibHE4SUtil.requiredUserVariants.length; i++) {
    const variant = LibHE4SUtil.requiredUserVariants[i];
