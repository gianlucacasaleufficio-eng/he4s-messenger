...
export async function encrypt(
  ...
  if (encryptForClosedGroup) {
    // window?.log?.info(
    //   'Encrypting message with HE4SProtocol and envelope type is CLOSED_GROUP_MESSAGE'
    // );
    ...
    const cipherTextClosedGroup = await MessageEncrypter.encryptUsingHE4SProtocol(
      hexPubFromECKeyPair,
      plainText
    );
  ...
  const cipherText = await MessageEncrypter.encryptUsingHE4SProtocol(device, plainText);

  return { envelopeType: SESSION_MESSAGE, cipherText };
}

export async function encryptUsingHE4SProtocol(
  ...
  // window?.log?.info('encryptUsingHE4SProtocol for ', recipientHexEncodedX25519PublicKey.key);
