...
import { OpenGroupPollingUtils } from '../../../../he4s/apis/open_group_api/opengroupV2/OpenGroupPollingUtils';
...
import { Onions } from '../../../../he4s/apis/snode_api/onions';
import { getConversationController } from '../../../../he4s/conversations/ConversationController';
import { MessageEncrypter } from '../../../../he4s/crypto';
import { OnionSending } from '../../../../he4s/onions/onionSend';
import { OnionV4 } from '../../../../he4s/onions/onionv4';
import { MessageSender } from '../../../../he4s/sending';
import { PubKey, RawMessage } from '../../../../he4s/types';
import { MessageUtils, UserUtils } from '../../../../he4s/utils';
import { fromBase64ToArrayBuffer } from '../../../../he4s/utils/String';
...
describe('MessageSender', () => {
  ...
  describe('send', () => {
    ...
    let he4sMessageAPISendStub: sinon.SinonStub<any>;
    let encryptStub: sinon.SinonStub<[PubKey, Uint8Array, SignalService.Envelope.Type]>;

    beforeEach(() => {
      he4sMessageAPISendStub = Sinon.stub(MessageSender, 'sendMessagesDataToSnode').resolves();
    ...
    describe('retry', () => {
      ...
      it('should not retry if an error occurred during encryption', async () => {
        ...
        expect(he4sMessageAPISendStub.callCount).to.equal(0);
      });
      ...
      it('should only call lokiMessageAPI once if no errors occured', async () => {
        ...
        expect(he4sMessageAPISendStub.callCount).to.equal(1);
      });

      it('should only retry the specified amount of times before throwing', async () => {
        he4sMessageAPISendStub.throws(new Error('API error'));
        const attempts = 2;
        const promise = MessageSender.send({
          message: rawMessage,
          attempts,
        ...
        expect(he4sMessageAPISendStub.callCount).to.equal(attempts);
      });

      it('should not throw error if successful send occurs within the retry limit', async () => {
        he4sMessageAPISendStub.onFirstCall().throws(new Error('API error'));
        await MessageSender.send({
          message: rawMessage,
          attempts: 3,
          retryMinTimeout: 10,
        ...
        expect(he4sMessageAPISendStub.callCount).to.equal(2);
      });
    });
    ...
    describe('logic', () => {
      ...
      it('should pass the correct values to lokiMessageAPI', async () => {
        ...
        const args = he4sMessageAPISendStub.getCall(0).args;
      ...
      it('should correctly build the envelope and override the timestamp', async () => {
        ...
        const firstArg = he4sMessageAPISendStub.getCall(0).args[0];
