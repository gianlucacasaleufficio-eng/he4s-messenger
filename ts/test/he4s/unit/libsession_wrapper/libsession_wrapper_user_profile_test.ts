...
describe('libhe4s_user_profile', () => {
  ...
  describe('isUserProfileToStoreInWrapper', () => {
    it('returns true if thats our convo', () => {
      expect(HE4SUtilUserProfile.isUserProfileToStoreInWrapper(ourNumber)).to.be.true;
    });

    it('returns false if thats NOT our convo', () => {
      const notUs = TestUtils.generateFakePubKeyStr();
      expect(HE4SUtilUserProfile.isUserProfileToStoreInWrapper(notUs)).to.be.false;
    });
  });
  ...
  describe('insertUserProfileIntoWrapper', () => {
    ...
    it('returns wrapper values that match with the inputted user profile', async () => {
      ...
      Sinon.stub(HE4SUtilUserProfile, 'isUserProfileToStoreInWrapper').returns(true);

      const wrapperUserProfile =
        await HE4SUtilUserProfile.insertUserProfileIntoWrapper(ourNumber);
